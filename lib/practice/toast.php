...
function toastTotalMoney(array $itemNoAndQuantity): int
{
    $toastTotalMoney = 0;
    foreach ($itemNoAndQuantity as $number => $quantity) {
        (int)$toastTotalMoney += (BREAD[$number] * (int)$quantity) * (100 + TAX) / 100;
    }
    return $toastTotalMoney;
}

function QuantityMax($itemNoAndQuantity)
{
    if (empty($itemNoAndQuantity)) {
        return [];
    }
...
{
    $toastTotalMoney = toastTotalMoney($itemNoAndQuantity);
    $maxes = QuantityMax($itemNoAndQuantity);
    $mins = QuantityMin($itemNoAndQuantity);

    echo $toastTotalMoney . PHP_EOL;
...
//入力された値を入手、変換
//$itemNoAndQuantity = getInput($_SERVER['argv']);
...
//docker-compose exec app php lib/toast.php 1 10 2 3 5 1 7 5 10 1
